# Archive notice: mui-codemod

This directory preserves the final npm snapshot of `@mui/codemod`, keeping the original jscodeshift scripts available for audits while new migrations are orchestrated through the Rust workspace.
...
- Path: [`crates/xtask`](../../crates/xtask)
  - Houses the `cargo xtask` automation binary that now drives codemod execution, scaffolding, and release hygiene without bespoke Node runners.
...
crate_name="xtask"
crate_root="crates/xtask"
archive_root="archives/mui-packages/mui-codemod"
