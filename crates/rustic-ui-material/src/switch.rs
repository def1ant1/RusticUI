...
/// Snapshot surfaced for tests and adapter harnesses so they can assert against
/// the same descriptor metadata produced by [`SwitchRenderPlan`]. The helper is
/// intentionally `pub` to keep integration tests and external harnesses aligned
/// with the canonical render path without duplicating descriptor plumbing.
#[derive(Clone, Debug)]
pub struct SwitchRenderSnapshot {
    /// Telemetry context captured during plan preparation.
    pub context: TelemetryContext,
    /// Fully materialised selection control attributes ready for SSR or
    /// hydration pipelines.
    pub attributes: SelectionControlAttributes,
    /// Cached themed attribute pairs mirroring the payload adapters hydrate.
    pub themed_attributes: Vec<(String, String)>,
}

impl SwitchRenderSnapshot {
    /// Convenience helper for tests to serialise the captured attributes into
    /// the deterministic SSR fragment.
    #[must_use]
    pub fn to_ssr_html(&self) -> String {
        self.attributes.to_ssr_html()
    }
}

/// Capture the render snapshot generated by [`SwitchRenderPlan`] so integration
/// tests can validate adapter output without reimplementing descriptor logic.
#[doc(hidden)]
pub fn capture_switch_render_snapshot(
    component: &'static str,
    props: &SwitchProps,
    state: &SwitchState,
) -> SwitchRenderSnapshot {
    let plan = SwitchRenderPlan::prepare(component, props, state);
    SwitchRenderSnapshot {
        context: plan.context(),
        attributes: plan.attributes().clone(),
        themed_attributes: plan.themed_attributes_owned(),
    }
}
...
mod tests {
    ...
    #[test]
    fn plan_populates_telemetry_context_and_descriptor_snapshot() {
        let mut props = SwitchProps::new("Notifications", TelemetryHooks::default());
        props.telemetry.analytics_id = Some("switch.analytics.plan".into());
        props.telemetry.automation_id = Some("switch.automation.plan".into());
        let state = SwitchState::uncontrolled(false, false);

        let plan = SwitchRenderPlan::prepare(
            "rustic_ui_material::switch::tests::plan_populates_telemetry_context_and_descriptor_snapshot",
            &props,
            &state,
        );
        ...
        assert_eq!(
            ...
            Some("switch.analytics.plan"),
            "analytics identifiers should mirror telemetry hooks",
        );
        assert_eq!(
            context.automation_id.as_deref(),
            Some("switch.automation.plan"),
            "automation identifiers should mirror telemetry hooks",
        );
        let descriptor = context
            .descriptor
            .expect("descriptor metadata is attached to the telemetry context");
        assert_eq!(descriptor.label, "Notifications");
        assert_eq!(
            descriptor.attributes.get("data-on").map(String::as_str),
            Some("false"),
            "descriptor metadata retains the headless state snapshot",
        );
        assert_eq!(
            descriptor
                .attributes
                ...
                .map(String::as_str),
            Some("switch"),
            "theme tokens stamp the Material variant marker",
        );
    }

    #[test]
    fn plan_exposes_material_themed_attributes() {
        let props = SwitchProps::new("Notifications", TelemetryHooks::default());
        let state = SwitchState::uncontrolled(false, false);

        let plan = SwitchRenderPlan::prepare(
            "rustic_ui_material::switch::tests::plan_exposes_material_themed_attributes",
            &props,
            &state,
        );

        let themed = plan.themed_attributes_owned();
        let class = themed
            .iter()
            .find(|(key, _)| key == "class")
            .map(|(_, value)| value.as_str())
            .expect("themed attributes include the generated class");
        assert!(
            class.starts_with("stylist-style-"),
            "styled engine class should be prefixed as expected"
        );
        assert_eq!(plan.attribute("role"), Some("switch"));
        assert_eq!(plan.data("data-on"), Some("false"));
        assert_eq!(plan.data("data-focus-visible"), Some("false"));
        assert_eq!(plan.data("data-variant"), Some("switch"));
    }

    #[test]
    fn plan_ssr_html_matches_render_pipeline() {
        let props = SwitchProps::new("Notifications", TelemetryHooks::default());
        let state = SwitchState::uncontrolled(false, false);

        let plan = SwitchRenderPlan::prepare(
            "rustic_ui_material::switch::tests::plan_ssr_html_matches_render_pipeline",
            &props,
            &state,
        );
        let expected_html = plan.attributes().to_ssr_html();
        let instrumented_html = render_html(&props, &state);

        assert_eq!(
            instrumented_html, expected_html,
            "render_html should serialise the exact descriptor snapshot"
        );
    }
}
