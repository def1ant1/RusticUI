...
| Example | Renderer coverage | Automation & test focus | Bootstrap / build commands |
| --- | --- | --- | --- |
| `selection-controls-react` | `npm run dev` orchestrates `wasm-pack --watch` with the Vite dev server so React hydrates against live wasm bindings. | Telemetry delegates stream JSON events that Jest and Playwright assert for lifecycle/user ordering. | `just bootstrap`, `just build`, and `just test` wrap npm scripts plus host/wasm Cargo builds for CI reuse. |
