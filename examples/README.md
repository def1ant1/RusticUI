...
| Example | Renderer coverage | Automation & test focus | Bootstrap / build commands |
| --- | --- | --- | --- |
| `selection-controls-react` | `just dev` launches `wasm-pack` in watch mode alongside the Vite server so telemetry logs stream in real time while editing.【F:examples/selection-controls-react/README.md†L33-L75】 | Jest, wasm-bindgen, and Playwright harnesses assert that telemetry delegates emit JSON events before React callbacks run.【F:examples/selection-controls-react/README.md†L47-L92】 | `just bootstrap`, `just build`, `just test`, and `just automation-smoke` wrap the shared smoke script and npm flows for CI parity.【F:examples/selection-controls-react/README.md†L33-L75】 |
| `selection-controls-yew` | `just serve` hydrates the SSR snapshots with telemetry consoles while `cargo run` prints the annotated HTML fragments for review.【F:examples/selection-controls-yew/README.md†L9-L42】 | Host + wasm tests verify the logging order (`TelemetryHooks` before public callbacks) and ensure automation IDs stay deterministic.【F:examples/selection-controls-yew/README.md†L33-L60】 | `just`, `cargo host-check`, `cargo wasm-test`, and `just automation-smoke` delegate to the shared smoke harness so CI and local workflows stay aligned.【F:examples/selection-controls-yew/README.md†L9-L60】 |
| `utils-trap-focus-*` | `cargo run --bin bootstrap --manifest-path examples/utils-trap-focus-{framework}/Cargo.toml` emits deterministic SSR snapshots and hydration stubs for Yew, Leptos, Dioxus, and Sycamore harnesses.【F:examples/utils-trap-focus-yew/README.md†L9-L17】【F:examples/utils-trap-focus-leptos/README.md†L9-L17】【F:examples/utils-trap-focus-dioxus/README.md†L9-L17】【F:examples/utils-trap-focus-sycamore/README.md†L9-L17】 | Unit tests confirm the start/end sentinels and automation hooks persist across SSR + hydration.【F:examples/utils-trap-focus-yew/README.md†L11-L13】【F:examples/utils-trap-focus-leptos/README.md†L11-L13】【F:examples/utils-trap-focus-dioxus/README.md†L11-L13】【F:examples/utils-trap-focus-sycamore/README.md†L11-L13】 | `./examples/utils-trap-focus-{framework}/scripts/bootstrap.sh` ensures toolchains are provisioned before regenerating assets.【F:examples/utils-trap-focus-yew/README.md†L19-L22】【F:examples/utils-trap-focus-leptos/README.md†L19-L22】【F:examples/utils-trap-focus-dioxus/README.md†L19-L22】【F:examples/utils-trap-focus-sycamore/README.md†L19-L22】 |
| `selection-controls-leptos` | Example component wires Material adapters into a typed Leptos view while telemetry helpers log to `leptos::logging` with automation IDs (`automation.selection-controls.*`).【F:examples/selection-controls-leptos/README.md†L1-L58】 | Demonstrates controlled/uncontrolled flows plus telemetry delegates that stream lifecycle events for SSR + CSR parity.【F:examples/selection-controls-leptos/README.md†L1-L94】 | Pair with the shared smoke script via `examples/scripts/selection-controls-smoke.sh leptos --mode smoke` once embedded into an app; update automation IDs in both locations when extending channels.【F:examples/selection-controls-leptos/README.md†L1-L58】【F:examples/scripts/selection-controls-smoke.sh†L32-L63】 |
| `selection-controls-dioxus` | Dioxus harness exposes Material render plans with telemetry logging before state transitions, mirroring the script comments about deterministic ordering.【F:examples/selection-controls-dioxus/README.md†L1-L27】【F:examples/scripts/selection-controls-smoke.sh†L1-L63】 | Controlled `rsx!` template documents analytics payload shapes so QA can diff SSR/CSR markup safely.【F:examples/selection-controls-dioxus/README.md†L1-L27】 | `examples/scripts/selection-controls-smoke.sh dioxus --mode serve --port 4703` launches the automation server; `--mode smoke` runs the CI-ready telemetry checks.【F:examples/scripts/selection-controls-smoke.sh†L17-L63】 |
| `selection-controls-sycamore` | Sycamore adapter consumes the same render plans and logs telemetry using deterministic automation selectors noted in the smoke script.【F:examples/selection-controls-sycamore/README.md†L1-L30】【F:examples/scripts/selection-controls-smoke.sh†L32-L63】 | Highlights parity between SSR snapshots and client hydration, including analytics delegates and radio commit payloads.【F:examples/selection-controls-sycamore/README.md†L1-L30】 | Use the shared smoke script with `sycamore` mode or run `cargo xtask selection-controls --skip-web` when validating Rust-only integrations.【F:crates/xtask/src/main.rs†L163-L2381】【F:examples/scripts/selection-controls-smoke.sh†L17-L63】 |
