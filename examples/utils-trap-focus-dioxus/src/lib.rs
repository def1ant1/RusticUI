//! Dioxus harness that reuses the shared focus trap story for SSR + hydration.

use dioxus::prelude::*;
use rustic_ui_material::focus_trap::dioxus as focus_trap_dioxus;
...
/// Render the focus trap surface by stitching together the shared sentinel HTML.
pub fn trap_focus_markup() -> String {
    ...
    let start = focus_trap_dioxus::render(&focus_trap_dioxus::FocusTrapSentinelProps {
        state: story.cloned_state(),
        kind: FocusTrapSentinelKind::Start,
        options: options.clone(),
        fallback_prefix: fallback_prefix.clone(),
    });
    let end = focus_trap_dioxus::render(&focus_trap_dioxus::FocusTrapSentinelProps {
        state: story.cloned_state(),
...
/// Hydration harness used by the WebAssembly runtime.
#[component]
pub fn TrapFocusHarness(cx: Scope) -> Element {
...
mod tests {
    ...
    #[test]
    fn ssr_markup_contains_sentinals() {
