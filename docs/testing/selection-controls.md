...
- **GitHub Actions** – The `Selection Controls Matrix` job inside `rust-ci.yml` now runs `cargo xtask verify-toolchain`, `cargo xtask fmt --check`, `cargo xtask clippy`, `cargo xtask selection-controls --skip-web`, `cargo xtask docs-build`, `cargo xtask docs-test`, and `cargo xtask docs-package --dry-run`. The workflow keeps CI Rust-only while still rebuilding and validating the documentation bundle on every run.【F:.github/workflows/rust-ci.yml†L180-L236】
- **Netlify** – The build command shells directly through `cargo xtask docs-package`, ensuring the mdBook export and wasm bundle are always generated by Rust automation.【F:netlify.toml†L1-L11】
- **Vercel** – Preview deployments mirror Netlify by invoking `cargo xtask docs-package`, keeping host-specific configuration trivial.【F:vercel.json†L1-L8】
