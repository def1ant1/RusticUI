...
function resolveArchivedMuiPath(alias: string, options: { subPath?: string } = {}) {
  ...
  if (!fs.existsSync(candidate)) {
    throw new Error(
      `Unable to resolve archived source for ${alias}. Expected ${candidate} to exist.\n` +
        'Inspect archives/README.md for the archival contract and regenerate the Rust-first shims if needed.',
    );
  }
...
const archivedDocsMarkdownSources = {
  material: [
    resolveArchivedMuiPath('@mui/lab'),
    resolveArchivedMuiPath('@mui/material'),
  ],
  joy: [resolveArchivedMuiPath('@mui/joy')],
};
    ...
    return {
      ...
      resolve: {
        ...
        alias: {
          ...
          // for 3rd party packages with dependencies in this repository. The Rust crates under
          // `crates/rustic-ui-*` own the runtime implementations; these aliases point to the
          // archived JavaScript snapshots so Next.js can transpile legacy demos until the wasm
          // shims generated by `cargo xtask docs-package` are wired into the bundler.
          '@mui/material$': resolveArchivedMuiPath('@mui/material', { subPath: 'src/index.js' }),
          '@mui/material': resolveArchivedMuiPath('@mui/material'),

          '@mui/docs': resolveArchivedMuiPath('@mui/docs'),
          '@mui/icons-material$': resolveArchivedMuiPath('@mui/icons-material', {
            subPath: 'lib/esm/index.js',
          }),
          '@mui/icons-material': resolveArchivedMuiPath('@mui/icons-material', { subPath: 'lib/esm' }),
          '@mui/lab': resolveArchivedMuiPath('@mui/lab'),
          '@mui/styled-engine': resolveArchivedMuiPath('@mui/styled-engine'),
          '@mui/system': resolveArchivedMuiPath('@mui/system'),
          '@mui/private-theming': resolveArchivedMuiPath('@mui/private-theming'),
          '@mui/utils': resolveArchivedMuiPath('@mui/utils'),
          '@mui/material-nextjs': resolveArchivedMuiPath('@mui/material-nextjs'),
          '@mui/joy': resolveArchivedMuiPath('@mui/joy'),
        },
        extensions: [
          '.tsx',
          // @ts-ignore
                ...
                use: [
                  options.defaultLoaders.babel,
                  {
                    ...
                    options: {
                      ...
                      // The Markdown loader hydrates demos using the archived JS sources while the
                      // Rust crates generate authoritative documentation via `cargo xtask docs-package`.
                      // Keep these arrays derived from the filesystem so new archives are picked up
                      // automatically without touching webpack config.
                      packages: [
                        {
                          productId: 'material-ui',
                          paths: archivedDocsMarkdownSources.material,
                        },
                        {
                          productId: 'joy-ui',
                          paths: archivedDocsMarkdownSources.joy,
                        },
                      ],
                      env: {
